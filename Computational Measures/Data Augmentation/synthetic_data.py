...
path='/content/drive/MyDrive/Nadiyah/final_codes'
...
def create_windows(df, Y, window_size, step_size=1, forecast_horizon=1):
  x=[]
  y=[]

  for i in range(0, len(df)-window_size-forecast_horizon, step_size):
    x.append(df.iloc[i:i+ window_size])
    y.append(Y.iloc[i+window_size:i+window_size+forecast_horizon])

  return np.array(x), np.array(y)
  
  
X, Y= create_windows(data, y, window_size=10, step_size=10)
...
def save_file(list_, filename):
  with open(path+ f'/loss_curves/data_augmentation/{filename}', 'wb') as f:
    dill.dump(list_, f)
...
def run_model(model, x_train, y_train, x_test, y_test, epochs, device, learning_rate):
  train_loss=[]
  val_loss=[]
...
metadata=pd.read_csv(path+ "/loss_curves/data_augmentation/metadata_aug.csv")
...
while (i<4):
  ...
  model_, train_loss, val_loss =run_model(model, x_train, y_train, x_test,
                                                            y_test, 100, device, learning_rate=0.001)
  ...
  for x_test, y_test in data_loader:
    ...
    mse=torch.mean(torch.square(predictions-y_test))
  print("MSE: ", mse)
  mean_orig.append(mse)
  i+=1


  metadata=add_metadata("Transformers: Training without feat_imp", i, "Synthetic Data", 100, mse.item())


save_file(orig_loss, 'syn_data_trans_loss.dill')
save_file(orig_val_loss, 'syn_data_trans_val_loss.dill')
save_file(mean_orig, 'syn_data_trans_mse.dill')     



#training with feature_imps for Transformers
with open(path+ '/shap_values/syn_data_trans_shap_values.dill', 'rb') as f:
...
while(i<4):
  ...
  model_, train_loss, val_loss =run_model(model, x_train, y_train, x_test, y_test, 100, device, learning_rate=0.001)
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("Transformers: Training with feat_imp", i, "Synthetic Data", 100, mse.item())


  i+=1


save_file(new_loss, 'syn_data_trans_new_loss.dill')
save_file(new_val_loss, 'syn_data_trans_new_val_loss.dill')
save_file(mse_new, 'syn_data_trans_mse_new.dill')
...
while(i<4):
  model=customRNN(10, 6)
  x_train, x_test, y_train, y_test= train_test_split(X, Y, test_size=0.3,
                                                     shuffle=False)
  model.to(device)
  model_, train_loss, val_loss =run_model(model, x_train, y_train, x_test, y_test, 100, device, learning_rate=0.01)
  ...
  print("MSE: ", mse)
  mse_orig.append(mse)
  metadata=add_metadata("RNN: Training without feat_imp", i, "Synthetic Data", 100, mse.item())

  i+=1


save_file(orig_loss, 'syn_data_rnn_loss.dill')
save_file(orig_val_loss, 'syn_data_rnn_val_loss.dill')
save_file(mse_orig, 'syn_data_rnn_mse.dill') 



#training with feat_imps RNN
with open(path+ '/shap_values/syn_data_rnn_shap_values.dill', 'rb') as f:
  shap_values=dill.load(f)
...
while(i<4):
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("RNN: Training with feat_imp", i, "Synthetic Data", 100, mse.item())

  i+=1


save_file(new_loss, 'syn_data_rnn_new_loss.dill')
save_file(new_val_loss, 'syn_data_rnn_new_val_loss.dill')
save_file(mse_new, 'syn_data_rnn_mse_new.dill')



metadata.to_csv(path+ "/loss_curves/data_augmentation/metadata_aug.csv")
