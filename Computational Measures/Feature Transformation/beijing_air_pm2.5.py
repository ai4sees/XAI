...
path='/content/drive/MyDrive/MyWork/final_codes/'
...
beijing_air_data, beijing_air_labels=load_data('beijing 2.5')

def save_file(list_, filename):
  with open(path+ f'/Table_3/feature_transformation/{filename}', 'wb') as f:
    dill.dump(list_, f)
...
metadata=pd.DataFrame(columns=["Model", "No. of Iteration", "dataset", "epochs", "MSE"])
...
#training without feature importances transformers 
while (i<3):
  # Create the model
  model = TransformerRegressor(input_dim, d_model, nhead, num_encoder_layers, dim_feedforward, output_dim)
  model.to(device)
  x_train, x_test, y_train, y_test= train_test_split(beijing_air_data, beijing_air_labels, test_size=0.3,
                                                     shuffle=False)
  ...
  metadata=add_metadata("Transformers: Training without feat_imp", i, "Beijing Air", 100, mse.item())


save_file(orig_loss, 'beijing_air_trans_loss.dill')
save_file(orig_val_loss, 'beijing_air_trans_val_loss.dill')
save_file(mean_orig, 'beijing_air_trans_mse.dill')     



#training with feature_imps for Transformers
with open(path+'shap_values/beijing_air_trans_shap', 'rb') as f:
...
while(i<4):
  model = TransformerRegressor(input_dim, d_model, nhead, num_encoder_layers, dim_feedforward, output_dim)
  model.to(device)
  new_data=beijing_air_data*shap_values
  x_train, x_test, y_train, y_test= train_test_split(new_data, beijing_air_labels, test_size=0.3,
                                                     shuffle=False)
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("Transformers: Training with feat_imp", i, "Beijing Air", 100, mse.item())
...
save_file(new_loss, 'beijing_air_trans_new_loss.dill')
save_file(new_val_loss, 'beijing_air_trans_new_val_loss.dill')
save_file(mse_new, 'beijing_air_trans_mse_new.dill')
...
while(i<4):
  ...
  print("MSE: ", mse)
  mse_orig.append(mse)
  metadata=add_metadata("RNN: Training without feat_imp", i, "Beijing Air", 100, mse.item())

  i+=1


save_file(orig_loss, 'beijing_air_rnn_loss.dill')
save_file(orig_val_loss, 'beijing_air_rnn_val_loss.dill')
save_file(mse_orig, 'beijing_air_rnn_mse.dill') 



#training with feat_imps RNN
with open(path+'shap_values/beijing_air_rnn_shap_values.dill', 'rb') as f:
  shap_values=dill.load(f)
...
while(i<4):
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("RNN: Training with feat_imp", i, "Beijing Air", 100, mse.item())

  i+=1


save_file(new_loss, 'beijing_air_rnn_new_loss.dill')
save_file(new_val_loss, 'beijing_air_rnn_new_val_loss.dill')
save_file(mse_new, 'beijing_air_rnn_mse_new.dill')



metadata.to_csv(path+ "/Table_3/feature_transformation/metadata_aug.csv")
