...
multi_site_data, multi_site_labels=load_data('beijing 2.5')
...
metadata=pd.read_csv("/content/drive/MyDrive/Nadiyah/final_codes/Table_3/feature_transformation/metadata_aug.csv")
...
while (i<3):
  ...
  metadata=add_metadata("Transformers: Training without feat_imp", i, "Beijing Air Multi Site",
   100, mse.item())
...
with open(path+'shap_values/multi_site_trans_shap', 'rb') as f:
...
while(i<4):
  model = TransformerRegressor(input_dim, d_model, nhead, num_encoder_layers, dim_feedforward, output_dim)
  model.to(device)
  new_data=multi_site_data*shap_values
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("Transformers: Training with feat_imp", i, "Beijing Multi site", 100, mse.item())
...
while(i<4):
  ...
  print("MSE: ", mse)
  mse_orig.append(mse)
  metadata=add_metadata("RNN: Training without feat_imp", i, "Beijing Mutli Site", 100, mse.item())
...
while(i<4):
  ...
  print("MSE: ", mse)
  mse_new.append(mse)
  metadata=add_metadata("RNN: Training with feat_imp", i, "Beijing Multi Site", 100, mse.item())
