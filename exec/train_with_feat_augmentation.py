...
if args.model == "rnn":
    model = RNNModel((df.shape[1], 2*df.shape[2]))

if args.model == "cnn":
    model = CNNModel((df.shape[1], 2*df.shape[2]))

if args.model == "dnn":
    model = DNNModel(df.shape[1], 2*df.shape[2])
...
xai_ = Xai(model, back_data, df, labels)
M, aug_train_loss, aug_val_loss = xai_.train_with_feat_aug(feat_cont,
...
plt.plot(range(args.epochs), aug_train_loss, label="train loss with transformed data")
plt.plot(range(args.epochs), aug_val_loss, label="validation loss with transformed data")
plt.legend()
plt.ylim(0, 1)
plt.savefig("loss_curves_augmentation.png")